...
def extract_data_IESA_multiple(results_file_path, intervals, list_sheets, nrows, filters, headers):
    """
    ...
    Args:
        ...
        results_file_path (str or Path): Path to the Excel file
    ...
    for interval in intervals:
        for sheet in list_sheets:
            results_dict[f"results_{interval}_{sheet}"] = []

    # Loop over each interval and sheet
    for i in range(len(intervals)):
        for j in range(len(list_sheets)):
            # Read specific sheet from Excel file
            df = pd.read_excel(results_file_path, sheet_name=list_sheets[j], nrows=nrows[j], header=0)
            ...
            for filter_values in sheet_filters:
                ...
                results_dict[f"results_{intervals[i]}_{list_sheets[j]}"].append(filter_entry)
