...
def set_max_import(modelhub, period, carrier_import_dict):
    """
        Adds constraints to the model to enforce maximum annual import for specific energy carriers.

        For each carrier in `carrier_import_dict`, this function constrains the total imported product
        (summed over all nodes, technologies, and modeled time steps) to smaller than or equal to the specified annual
        maximum, adjusted by the fraction of the year being modeled.
